...
func newWatcherMgr(ctx context.Context, l *Locator, key string, insKinds ...known.Kind) (*watcherMgr, error) {
	sub := l.client.Subscribe(ctx)
	channels := make([]string, 0, len(insKinds))
	for _, insKind := range insKinds {
		channels = append(channels, fmt.Sprintf(channelEventKey, l.prefix, string(insKind)))
	}
